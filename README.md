...
示例：登录获取 token（后续受保护接口都需要这个 token）。
...
示例：充值（操作主体由 token 解析，body 中无需也不应包含 `username`）。

```bash
curl -X POST http://127.0.0.1:8000/api/wallet \
  -H "Authorization: Bearer $TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{"asset":"USDT","amount":"10000","action":"deposit"}'
```
...
curl -X POST http://127.0.0.1:8000/api/order \
  -H "Authorization: Bearer $TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{"pair":"BTC/USDT","side":"BUY","price":"30000","quantity":"0.1","type":"LIMIT"}'
```
...
curl -X POST http://127.0.0.1:8000/api/order \
  -H "Authorization: Bearer $TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{"pair":"BTC/USDT","side":"BUY","quantity":"0.1","type":"MARKET"}'
```
