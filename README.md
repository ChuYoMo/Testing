...
一个用于《软件测试》课程实验的简化去中心化交易所原型。项目使用 Python 标准库实现用户认证、多币种钱包、限价单撮合、成交结算、链式账本记录、浏览器 GUI 和自动化测试，适合用于课程演示、测试设计和软件文档编写。
...
| 订单撮合 | 限价买单、限价卖单、订单簿、价格优先、时间优先、部分成交、完全成交、自成交保护 |
...
| 链式账本   | 创世区块、成交记录待打包、达到容量自动封块、手动封块、链完整性校验 |
| 命令行演示 | `app.py::run_demo()` 运行固定交易流程并返回结构化结果        |
>>>>>>> 3ab40b93de610f12cd91b9450f26a9599ba15a25
| 浏览器 GUI | 基于 `http.server` 的单页界面和 JSON API，可完成注册、登录、钱包、下单、封块、校验、重置和一键交易流程 |
| 自动化测试 | 使用 `unittest` 覆盖核心交易流程、GUI 流程、区块链快照隔离和金额合法性校验 |
...
- 注册和登录用户。
- 对指定用户执行充值、提现。
- 提交 `BTC/USDT`、`ETH/USDT`、`ETH/BTC` 限价买单或卖单。
- 一键执行 Bob/Alice/Carol 演示交易流程。
- 查看用户、钱包、订单、订单簿、交易流程、待打包交易和区块链。
- 手动封装待打包交易。
- 校验区块链完整性。
- 重置演示状态。
...
curl -X POST http://127.0.0.1:8000/api/order \
  -H 'Content-Type: application/json' \
  -d '{"username":"dave","pair":"BTC/USDT","side":"BUY","price":"30000","quantity":"0.1"}'
```
...
- 如果最优对手单属于同一用户，则触发自成交保护，本轮撮合停止，并在 `OrderPlacementResult.self_trade_blocked` 中标记。
...
- 没有真实钱包地址和私钥管理。
- 没有订单撤销接口，虽然 `OrderStatus.CANCELLED` 已定义。
- 没有手续费模型。
- 没有市价单。
- 没有并发锁或事务机制。
...
3. 撮合模块独立单元测试：价格优先、时间优先、自成交保护、不支持交易对。
4. 区块链模块独立单元测试：空封块、自动封块、篡改交易、篡改哈希。
5. 功能扩展：撤单、手续费、历史订单、文件或 SQLite 持久化。
